...
def parse_dir_name(dir_name: str) -> float:
    parts = dir_name.split("_")
    if len(parts) < 3:
        raise ValueError(f"Directory name has too few parts: {dir_name}")

    try:
        cx_idx = parts.index("cxPb")
    except ValueError as exc:
        raise ValueError(f"Could not parse cxPb from: {dir_name}") from exc

    if cx_idx + 1 >= len(parts):
        raise ValueError(f"Could not parse probability values from: {dir_name}")

    cx_pb = float(parts[cx_idx + 1])
...
def collect_runs(base_dir: Path) -> list[RunRecord]:
    ...
    for entry in sorted(base_dir.iterdir()):
        ...
        cx_pb = parse_dir_name(entry.name)
...
def summarize(runs: list[RunRecord]) -> list[dict[str, float | int | str]]:
    grouped: dict[tuple[float], list[RunRecord]] = defaultdict(list)
    for run in runs:
        grouped[(run.cx_pb,)].append(run)

    summary: list[dict[str, float | int | str]] = []
    for (cx_pb,), group_runs in sorted(grouped.items()):
