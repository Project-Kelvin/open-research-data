...
def parse_dir_name(dir_name: str) -> tuple[float, float, float]:
    ...
    cxIdx: int = 7
    reg = r".+(\d\.\d)"
    mutPbGrp = re.search(reg, parts[mutIdx])
    indPbGrp = re.search(reg, parts[indIdx])
    cxPbGrp = re.search(reg, parts[cxIdx])
    if not mutPbGrp or not indPbGrp or not cxPbGrp:
        raise ValueError(f"Could not parse mutPb/indPb/cxPb from: {dir_name}")
    mutation_pb = float(mutPbGrp.group(1))
    individual_pb = float(indPbGrp.group(1))
    crossover_pb = float(cxPbGrp.group(1))

    return mutation_pb, individual_pb, crossover_pb
...
def collect_runs(base_dir: Path) -> list[RunRecord]:
    ...
    for entry in sorted(base_dir.iterdir()):
        ...
        mutation_pb, individual_pb, crossover_pb = parse_dir_name(entry.name)
...
def summarize(runs: list[RunRecord]) -> list[dict[str, float | int | str]]:
    grouped: dict[tuple[float, float, float], list[RunRecord]] = defaultdict(list)
    for run in runs:
        grouped[(run.mutation_pb, run.individual_pb, run.crossover_pb)].append(run)

    summary: list[dict[str, float | int | str]] = []
    for (mutation_pb, individual_pb, crossover_pb), group_runs in sorted(grouped.items()):
