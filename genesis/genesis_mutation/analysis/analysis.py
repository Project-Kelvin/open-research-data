...
def parse_dir_name(dir_name: str) -> tuple[float, float]:
    parts = dir_name.split("_")
    if len(parts) < 3:
        raise ValueError(f"Directory name has too few parts: {dir_name}")

    try:
        mut_idx = parts.index("mutPb")
        ind_idx = parts.index("indPb")
    except ValueError as exc:
        raise ValueError(f"Could not parse mutPb/indPb from: {dir_name}") from exc

    if mut_idx + 1 >= len(parts) or ind_idx + 1 >= len(parts):
        raise ValueError(f"Could not parse probability values from: {dir_name}")

    mutation_pb = float(parts[mut_idx + 1])
    individual_pb = float(parts[ind_idx + 1])

    return mutation_pb, individual_pb
...
def collect_runs(base_dir: Path) -> list[RunRecord]:
    ...
    for entry in sorted(base_dir.iterdir()):
        ...
        mutation_pb, individual_pb = parse_dir_name(entry.name)
...
def summarize(runs: list[RunRecord]) -> list[dict[str, float | int | str]]:
    grouped: dict[tuple[float, float], list[RunRecord]] = defaultdict(list)
    for run in runs:
        grouped[(run.mutation_pb, run.individual_pb)].append(run)

    summary: list[dict[str, float | int | str]] = []
    for (mutation_pb, individual_pb), group_runs in sorted(grouped.items()):
