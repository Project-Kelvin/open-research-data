...
def parse_dir_name(dir_name: str) -> int:
...
def collect_runs(base_dir: Path) -> list[RunRecord]:
    ...
    for entry in sorted(base_dir.iterdir()):
        ...
        neurons = parse_dir_name(entry.name)
...
def summarize(runs: list[RunRecord]) -> list[dict[str, float | int | str]]:
    grouped: dict[tuple[int], list[RunRecord]] = defaultdict(list)
    for run in runs:
        grouped[(run.neurons,)].append(run)

    summary: list[dict[str, float | int | str]] = []
    for (neurons,), group_runs in sorted(grouped.items()):
