...
def parse_dir_name(dir_name: str) -> str:
    ...
    try:
        cx_idx = parts.index("activation")
    except ValueError as exc:
        raise ValueError(f"Could not parse activation from: {dir_name}") from exc

    if cx_idx + 1 >= len(parts):
        raise ValueError(f"Could not parse probability values from: {dir_name}")

    cx_pb = str(parts[cx_idx + 1])
...
def collect_runs(base_dir: Path) -> list[RunRecord]:
    ...
    for entry in sorted(base_dir.iterdir()):
        ...
        activation = parse_dir_name(entry.name)
...
def summarize(runs: list[RunRecord]) -> list[dict[str, float | int | str]]:
    ...
    for run in runs:
        grouped[(run.activation,)].append(run)

    summary: list[dict[str, float | int | str]] = []
    for (activation,), group_runs in sorted(grouped.items()):
